/**
 * MCP Integration Test (JavaScript version)
 *
 * Tests the complete MCP integration flow
 */

const { MCPHandler } = require('./core/mcp-handler');
const { MCPMethod } = require('./core/mcp-types');
...
async function runTests() {
    ...
    try {
        ...
        const response = await mcpHandler.handleRequest(listToolsRequest);
    ...
    try {
        ...
        const response = await mcpHandler.handleRequest(callToolRequest);
    ...
    try {
        ...
        if (content) {
            ...
            if (Array.isArray(results) && results.length > 0) {
                ...
                console.log('  Results:', results.map(r => `${r.path} (${r.matches} matches)`).join(', '), '\n');
                passed++;
            } else {
    ...
    try {
        ...
        const response = await mcpHandler.handleRequest(listResourcesRequest);
